...
namespace us4::runtime::backends::cpu_avx
{
    ...
    us4::core::Tensor Avx512BlockedMatMul(const us4::core::Tensor& left,
                                          const us4::core::Tensor& right,
                                          const BlockedMatMulPlan& plan)
    {
        if (left.Rank() != 2U || right.Rank() != 2U || left.Dim(1) != right.Dim(0))
        {
            throw std::invalid_argument("Avx512BlockedMatMul expects [M, K] x [K, N] tensors.");
        }
                     ...
                     depthBlockBegin += plan.tile.depthBlock)
                {
                    ...
                    for (std::size_t row = 0; row < rowBlock; ++row)
                    {
                        ...
                        float* outputRow = outputData + ((rowBlockBegin + row) * cols) +
                                           colBlockBegin;
                        std::size_t col = 0;
                        for (; col + 16U <= colBlock; col += 16U)
                        {
                            __m512 acc = _mm512_loadu_ps(outputRow + col);
                            for (std::size_t depth = 0; depth < depthBlock; ++depth)
                            {
                                const __m512 leftBroadcast = _mm512_set1_ps(
                                ...
                                const float* rightPanel = plan.packRightHandSide
                                                              ...
                                                              : rightData +
                                                                    ...
                                                                    colBlockBegin;
                                const __m512 rhs = _mm512_loadu_ps(rightPanel + col);
                                acc = _mm512_add_ps(acc, _mm512_mul_ps(leftBroadcast, rhs));
                            }
                            _mm512_storeu_ps(outputRow + col, acc);
                        }
