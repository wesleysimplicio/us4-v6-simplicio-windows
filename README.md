...
.\scripts\post-publish-smoke.ps1 -ArtifactPath .\dist\us4-v6-windows-0.1.33-portable.zip
...
.\scripts\render-winget-manifests.ps1 -Version 0.1.33
...
.\scripts\render-release-notes.ps1 -Version 0.1.33 -ReleaseManifestPath .\dist\release-manifest.json
...
.\scripts\sign-msix.ps1 -PackagePath .\dist\us4-v6-windows-0.1.33.0.msix
.\scripts\preflight-release.ps1 -BuildDir build
.\scripts\install-msix-smoke.ps1 -PackagePath .\dist\us4-v6-windows-0.1.33.0.msix
```
...
`dev-msix-smoke.ps1` is a local dev-only path. It reduces the MSIX gap by exercising self-signed packaging on a single host, but it does not satisfy the production requirement for a trusted release certificate.
