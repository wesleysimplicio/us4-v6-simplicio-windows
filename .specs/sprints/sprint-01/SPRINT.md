...
- [x] T01.7 — `.github/workflows/{ci,dod}.yml` (Win runner, MSVC + clang-cl, GoogleTest, Playwright smoke, coverage gate 80%)
- [x] T01.8 — `.claude/hooks/{post-edit,pre-commit}.sh` (lint/format + block red commits)
- [x] T01.9 — Playwright config + first smoke (`us4.exe --version` + `--probe` outputs vendor/backend chosen)
- [x] T01.10 — ADR-001 Adapter interface; ADR-002 Backend selection; ADR-003 Runtime mode
